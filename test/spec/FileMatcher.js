  ...
  describe("match", () => {
    ...
    describe("Not recursive w/o glob", () => {
      ...
      it("must throws Error when directory not exists", async () => {
        ...
        const matcher = new FileMatcher(config);
        await assertRejects(
          () => matcher.match("not-exists-file"),
          /no such file or directory 'not-exists-file'/
        );
      });
    });
    describe("Not recursive with glob", () => {
      it("must returns files, not includes directory", async () => {
        ...
        assert.deepEqual(
          files.sort(),
          ["scripts/file1.sh", "scripts/dir2/file2.js", "scripts/file3"].sort()
        );
      });
      it("must throws Error when directory not exists", async () => {
        const config = getConfig({ recursive: false });
        const matcher = new FileMatcher(config);
        await assertRejects(
          () => matcher.match("not-exists-dir/**/*"),
          /'not-exists-dir\/\*\*\/\*' is not found/
        );
      });
    });
    describe("recursive", () => {
